...
- **Установка zBot для CS 1.6**  
  - Установите официальный архив [`bot_profiles.zip`](https://github.com/rehlds/ReGameDLL_CS/raw/refs/heads/master/regamedll/extra/zBot/bot_profiles.zip) в каталог `cstrike`.
  - Укажите `bot_enable "1"` в `cstrike/game_init.cfg` и полностью перезапустите сервер.
  - Параметр командной строки `-bots` больше не используется: он удалён в пользу CVar `bot_enable`.
  - Команды, режимы квоты и навигация описаны в разделе [Использование ботов](./bots/).

- **Включение улучшенного AI заложников из CSCZ для CS 1.6**  
  - Установите необходимые ресурсы заложников.
  - Укажите `hostage_ai_enable "1"` в `cstrike/game_init.cfg` и полностью перезапустите сервер.
  - Устаревший параметр `-host-improv` удалён в пользу CVar `hostage_ai_enable`.
