...
Every project listed above now has a full documentation set — what it is, how to install it, every setting it exposes, how to build it from source, and who wrote it. And it exists in **seven languages**: English, Russian, Spanish, Brazilian Portuguese, Romanian, Polish and Simplified Chinese, 84 pages each.
...
On the documentation side, the shortlist we drew up from where live CS 1.6 servers actually are is now covered. Further languages are open to whoever wants one, but nothing is promised — translations only stay useful if somebody keeps them in sync, and a stale locale is worse than none at all.
