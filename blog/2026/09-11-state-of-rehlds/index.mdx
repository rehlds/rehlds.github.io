...
Every project listed above now has a full documentation set — what it is, how to install it, every setting it exposes, how to build it from source, and who wrote it. And it exists in **three languages**: English, Russian and Simplified Chinese, 84 pages each.
...
On the documentation side, the obvious next question is which language to add. Looking at where live CS 1.6 servers actually are rather than where we assume they are, Spanish and Brazilian Portuguese stand out, with Romanian and Polish close behind. Nothing is promised here — translations only stay useful if somebody keeps them in sync, and a stale locale is worse than none at all.
